"""
Script to download GFS, import into QGIS,and process
© Feb 18, 2019 - Chris Edwards, Jake Lewis, Hunter Williams
Modified for tethys implementation by Riley Hales & Chris Edwards May 22 2019
...
def process_new_gfs(request):
    """
    1. Download today's gfs forecast
    """
    # Specify the directory where the gfs files will be saved
    threddsdir = str(app_configuration()['threddsdatadir'])
    print(threddsdir)
    today = str(datetime.datetime.now().strftime("%Y%m%d"))
    downloadpath = os.path.join(threddsdir, 'gfs_' + today)
    ...
    for i in range(len(file_id_list)):
        # Download the Global file to the new raw-data directory:
        data_url = "https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?file=gfs.t00z.pgrb2.0p25.f" + file_id_list[
            i] + "&all_lev=on&var_APCP=on&leftlon=0&rightlon=360&toplat=90&bottomlat=-90&dir=%2Fgfs." + today

        filename = "gfs_apcp_" + today + "_t00_f" + file_id_list[i] + ".grib"
        download_file_path = Path(downloadpath + "/" + filename)
